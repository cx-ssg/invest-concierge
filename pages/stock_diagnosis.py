...
def calc_valuation_percentile(stock_code, years=5):
    """计算近 N 年 PE(TTM)/PB 历史分位（页面级展示工具）。

    数据源：优先 ak.stock_a_lg_indicator（乐咕乐股，个股 PE/PB 宽表，方案 §5.4-A 指定）；
...
def _load_diag_data(stock_code):
    """一次性统一拉取 5 引擎数据（财报引擎无缓存，务必只调一次；首次约 15-40s）。
