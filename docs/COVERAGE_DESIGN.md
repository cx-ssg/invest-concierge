...
3. **时效分层 + 检索参数显式固定（2026-09-14 修正）**：
   ...
   - **参数（不固定则 A2/A3 不可复现）**：向量 top-20 + **BM25** top-20 → RRF 融合（**k=60**，与知识库 `search_wiki.run_hybrid` 实现逐字一致）→ 取 top-5。
     **相关性双判据（2026-09-15 实测修正 —— 原写「相似度阈值 0.35」不成立）**：
     ① 绝对下限 `MIN_SIM = 0.45`：`max_sim` 低于它直接返回空（**A3 判据**）；
     ② 相对阈值 `MIN_SIM_RATIO = 0.85`：语义路只保留 `sim >= max_sim × 0.85` 的块。
     实测依据（bge-m3，5 篇样例）：**无关查询**的 `max_sim` = **0.30~0.38**（「量子计算」= 0.381，会被旧的 0.35 阈值漏过）；
     **相关查询**的相关块 = **0.72~0.80**、次相关块 ≈ 0.45 —— gap 明显，故绝对阈值只能做「有无」判断，块级取舍必须用相对阈值。
     ⚠️ 样本量小（5 篇，尚未接真实语料），接入公告/研报后需**重测并回填**。
