...
3. **时效分层 + 检索参数显式固定（2026-09-14 修正）**：
   ...
   - **参数（不固定则 A2/A3 不可复现）**：向量 top-20 + **BM25** top-20 → RRF 融合（**k=60**，与知识库 `search_wiki.run_hybrid` 实现逐字一致）→ 取 top-5。
     **相关性判据 = SAR + V1 双信号（2026-09-16 三次修正：向量绝对阈值方案已被实测证伪并替换）**：
     ① `SAR = bm25_max / (Σ_{t∈q} idf(t)·(k1+1))` —— BM25 分数达成率（Lucene maxScore/WAND 的解析上界归一化），**尺度无关**；
     ② `V1 = |{t∈q : 0 < df(t) ≤ max(1, N·0.05)}| / |{t∈q : df(t) ≤ max(1, N·0.05)}|` —— 特征词存在率。
     三档 `evidence_level`：**`none`**（弃权）/ **`weak`**（返回 + 标注证据不足，交 LLM 裁决）/ **`strong`**（正常返回）。
     **向量退回纯排序**（不再当闸门）。实现：`utils/rag/evidence.py`。

     **阈值与实测**（真实公告语料 814 块；工具 `scripts/rag_threshold_probe.py`，支持 `--holdout`）：
     初值 `SAR_STRONG=0.10 / V1_STRONG=0.45 / SAR_NONE=0.06 / V1_NONE=0.35`。

     | 查询组 | REL `sar` / `v1` | IRR `sar` / `v1` | 判定 |
     |---|---|---|---|
     | TUNING | 0.1103~0.2816 / 0.500~1.000 | 0.0000~0.0422 / 0.000~0.286 | 可分 ✅ |
     | **HOLDOUT（未参与定值）** | **0.1791~0.3651 / 0.571~0.818** | **0.0000~0.0490 / 0.000~0.222** | **可分 ✅** |

     🔑 **为什么换掉 `max_sim`**：① 它是**极值统计量** —— 语料越大越容易撞到高分近邻，误放行概率单调上升；
     ② 它衡量**话题风格接近度，而非"这块里有没有答案"** —— 留出组实测**区间倒挂**
     （无关「怎么写一封求职信」`max_sim=0.6902` > 相关「风险评估报告的结论」0.6668）。
     同一批查询在新判据下：该条 `sar=0.0000 / v1=0.0000 → none`（**确定性弃权**，无需阈值）。
     ⚠️ **阈值仍是实测初值**（仅 30 条查询）：按 `docs/M1_INGEST_AUDIT.md` §五 重建评测集
     （n≥50/类、含**域内无答案**负样本、A3 拆 a/b/c）后必须重定。
