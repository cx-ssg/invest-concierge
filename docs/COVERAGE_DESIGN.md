...
- 混合检索方案：FTS5/BM25 + 向量 + **RRF 融合**（知识库重构 A→D-lite 已跑通，含增量重建）
...
| 财报原文（PDF） | 巨潮资讯 / 东财 | 下载入 `data/corpus/pdf/` |
| 方法论（自建） | 项目 docs + 个人笔记 | 手工入 `data/corpus/notes/` |

### 3.2 数据流

```
[采集层] akshare 拉取 → 按股票代码 + 日期落盘 data/corpus/{code}/{source}/xxx.md|pdf
   ↓
[解析层] PDF → 文本（pdfplumber）；表格单独抽为结构化块（财报数字必须保留表结构）
   ↓
[切块层] 金融文档专用切块：按「章节标题 + 段落」切，表格整块不切碎；块大小 500-800 字，overlap 80
   ↓
[索引层] 每块 → bge-m3 向量 + FTS5 全文索引，存 SQLite（kb.db）
   ↓
[检索层] 查询 → 向量 top-k + FTS5 top-k → RRF 融合 → 重排（可选）→ top-n
   ↓
...
1. **为什么用 SQLite 而非专用向量库**：语料量级（单机万级块）下 SQLite + 暴力/分片检索足够；零部署、可随 exe 分发。若后期超 10 万块再考虑 sqlite-vec / hnswlib。
...
3. **时效分层 + 检索参数显式固定（2026-09-14 修正）**：
   ...
   - **参数（不固定则 A2/A3 不可复现）**：向量 top-20 + FTS5 top-20 → RRF 融合（**k=60**）→ 取 top-5；相似度阈值 **0.35**，低于阈值直接返回空 —— 这就是 A3「无关查询返回 0 条」的判据来源。
   - **索引构建必须后台化**：单机 CPU 上 embed 万级块耗时很长，`ingest` 一律后台跑，带进度、断点续跑与失败重试，**不得阻塞 UI**。
4. **无引用 = 不算回答**：生成层强制携带来源；检索为空时明确回「未找到相关公告」，**不允许模型凭空补**。
5. **增量更新**：按 `(code, source, date)` 幂等去重；每日增量拉取，不重建全库。
...
chunks_fts(text)                    -- FTS5 虚表
embeddings(chunk_id, model, dim, vec BLOB)
meta(key, value)                    -- 索引版本、模型名、构建时间
```
...
| 语料版权（研报） | 仅本地私用，不入仓库、不随 exe 分发（`data/corpus/` 进 .gitignore） |
| 向量模型在用户机器上不可用（无 ollama） | 降级链：本地 bge-m3 → 远程 API → 纯 FTS5 关键词检索（功能降级但不崩） |
...
| M1 检索 | ✅ **自研 SQLite + FTS5 + bge-m3 + RRF** | 已有成熟方案（知识库 A→D-lite）；Haystack / RAGFlow 的切块与 PDF 解析思路可参考，但不引入依赖 |
...
| P0-2 | 🟡 **部分完成（2026-09-15）**：`tool_end.ok` 判定已对齐真实错误契约（新增 `tool_output_is_error()`）；**per-tool 错误码仍待做** | M1 的 `retrieve_docs`、M3 的每个节点都踩在这层；不修则排障被误导 |
...
> **P0-1/P0-2 验收证据（2026-09-15）**：先写 RED（`tests/test_p0_agent_fixes.py` 8 条 → 4 failed）→ 修 → GREEN；全量 `pytest` **189 passed**（原 181）；同时修正了 `tests/test_m0_services.py` 里锁定**错误契约**的旧用例（它喂的是真实代码从不产生的「工具执行失败」字符串）。变更记录见 `CHANGELOG.md`。
...
- embedding 降级链（本地 bge-m3 → 远程 → 纯 FTS5）**从风险表一句话提升为验收项** —— 它是"零配置即用"卖点的守门员
